...
const tempTabsMap = new Map();

// chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
//   if (message.type === 'start') {
//     // 无效 chrome.tabCapture.capture undefined navigator.mediaDevices.getUserMedia undefined
...
function stopTempRecordTab(recordPageTabId) {
  chrome.tabs.sendMessage(tempTabsMap.get(recordPageTabId), {
    type: "stop",
    tabId: recordPageTabId,
  })
}

function closeTempRecordTab(recordPageTabId) {
  const tempTabId = tempTabsMap.get(recordPageTabId);
  if (tempTabId) {
    chrome.tabs.remove(tempTabId, () => {
      tempTabsMap.delete(recordPageTabId);
    });
  }
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === "start") {
    openTempRecordTab(message.tabId)
  } else if (message.type === "stop") {
    stopTempRecordTab(message.tabId)
  }else if(message.type === "closeTempRecordTab") {
    closeTempRecordTab(message.tabId)
  }
});
...
async function startRecording (tabId) {
  ...
  try {
    ...
    // const { width, height, audioBitsPerSecond, videoBitsPerSecond, mimeType, fps } = getConfig()
